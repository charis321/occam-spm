...
import { Table, Tag, Button, Tooltip, Input, Select, Form } from 'antd';
import {
  PlusCircleOutlined,
  SearchOutlined,
  EditOutlined,
...
export default function OCAdminUserManager(props) {
  const { user } = useAuth();
  ...
  const handleAddUserBatchSubmit = (e) => {
    e.preventDefault();
    console.log('提交批量新增用戶', userNewData);
  };
  ...
  const handleUserAction = (action, user) => {
    ...
    return (e) => {
      switch (action) {
        case 'check':
          navigator('/dashboard/admin/user/' + user.id);
          break;
        case 'edit':
          navigator('/dashboard/admin/user/' + user.id + '/edit');
          break;
        case 'activate':
          if (user.status === 1) {
            const user_patch = {
  ...
  return (
    <div className="oc-admin-user-manager">
  ...
  return (
    <div className="oc-user-new-block">
      <Form className="oc-user-new-form">
        <h2>新增用戶</h2>
        <Form.Item className="oc-user-new-form-item">
